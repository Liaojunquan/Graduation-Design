...
class Checking():
 ...
 def match_rate(self, str1, str2):  #获取相似率
    ...
    if ws['A'+str(i)].value != None and ws['A'+str(j)].value != None and ws['A'+str(i)].value.replace(' ','') == ws['A'+str(j)].value.replace(' ','') and ws['I'+str(i)].value == ws['I'+str(j)].value and ws['E'+str(i)].value == ws['E'+str(j)].value and ws['F'+str(i)].value == ws['F'+str(j)].value: #职位名称 公司名称 地区 经验要求均相同
     if ws['N'+str(i)].value == ws['N'+str(j)].value:   #发布日期相同，包括今日
      ...
      if ws['O'+str(i)].value != None and ws['O'+str(j)].value != None:
       ...
       tmp_rate, d_str = self.match_rate("", "")  #获取匹配率
      elif ws['O'+str(i)].value == None and ws['O'+str(j)].value != None:
       tmp_rate, d_str = self.match_rate("", ws['O'+str(j)].value)  #获取匹配率
      elif ws['O'+str(i)].value != None and ws['O'+str(j)].value == None:
       tmp_rate, d_str = self.match_rate(ws['O'+str(i)].value, "")  #获取匹配率
   ...
   if ws_n['P'+str(i)].value != None:
    ...
    for j in range(i+1, max_r + 1):
     if ws_n['P'+str(j)].value == P_index:  #查找到两行都有同一P标记
      ...
      ws_n['P'+str(i)] = None    #清除标记
  end_time = time.time()
  self.ui.log('查重完成! 用时' + str(int((end_time-start_time)/3600)) + '时' + str(int((end_time-start_time)/60)) + '分' + str(int((end_time-start_time)%60)) + '秒')
  wb.save(file_name)
  #wb.close()
   ...
   if ws['A'+str(i)].value != None:
    ...
    if j < len(kw_list):  #是否为相应职位
     if ws['F'+str(i)].value != None and ws['F'+str(i)].value.find('生') != -1:
      ...
      exp[8] += 1
  exp_list = ['在校生/应届生', '无需经验', '1年经验', '2年经验', '1-3年经验', '3-4年经验', '5-7年经验', '8-9年经验', '10年以上经验']
  if self.stop():  #提供停止
   return
   ...
   if ws['A'+str(i)].value != None:
    ...
    if j < len(kw_list):  #是否为相应职位
     if ws['G'+str(i)].value != None and ws['G'+str(i)].value.find('初中') != -1:
      ...
      edu[7] += 1
     elif ws['G'+str(i)].value != None and ws['G'+str(i)].value.find('无需') != -1:
      edu[8] += 1
  edu_list = ['初中及以下', '高中', '中专', '中技', '大专', '本科', '硕士', '博士', '无需经验']
  if self.stop():  #提供停止
   return
...
广告 小编 小红书 微博 抖音 B站 b站 编辑 Laya laya Egret egret"""
  ...
  animate_keyWord = "动画 动效 动作 flash Flash FLASH AN An an MG Mg mg 动漫 绑骨 绑定 pipeline Pipeline CG cg Cg"
  cs_keyWord = "C# c#"
  cpp_keyWord = "C++ c++"
  game_keyWord = "引擎 端游 手游 游戏 Unity U3D u3d U3d UE4 ue4 Ue4 虚幻 cocos Cocos COCOS Lua lua LUA Shader shader SHADER unity TA Ta ta 技术美术 技美 Laya laya Egret egret"
  ...
  model_keyWord = "建模 模型 maya Maya MAYA 3dmax 3DMAX 3DMax 3Dmax 3dMax 3dMAX ZB Zb zb ZBrush zbrush Zbrush ZBRUSH 犀牛 Blender blender 3ds 3Ds 贴图 材质" #建模
  photography_keyWord = "摄影 摄像 拍摄 录像 photograph 航拍 飞手" #摄影摄像
  post_production_keyWord = "渲染 C4D c4d C4d 特效 影视 后期 剪辑 分镜 合成 CG cg Cg 微电影 达芬奇 媒体 Davinci 调色 调光 字幕 栏目包装 地编" #影视后期
  sql_keyWord = "sql Sql SQL 数据库 DB DataBase Database database" #数据库
  technology_keyWord = "小程序 算法 数字信号 编码 解码 图像 图形"  #数媒技术
  unity_keyWord = "Unity U3D U3d u3d u3D unity UNITY Lua lua LUA Shader SHADER shader"  #Unity3D
...
class MainUi(UiRoot):
 ...
 def __init__(self):
  ...
  self.eachCityMoneyBtn = Checkbutton(visualizationFrame, text = '广东各城市数媒行业职位薪水对比', font = '6', command = None, variable = self.eachCityMoneyvar)  #1Bar
