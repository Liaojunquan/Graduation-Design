...
深厚 深刻 其他 其它 部门 规定 购买 社保 保险 聘 请 急 ü ● ★ ◆ ■ 丰富 富 周末 双休 者 能够 人员 具 分红 发放 广阔 一门 一次 一个 。 ° 一次 年终奖 周一 座 大厦 免费 或"""   #停用词表
  ...
  if self.stop():  #提供停止
   return False
  try:
   sw_file = open(self.this_path + '\\my_stopwords.txt', 'r', encoding = 'utf-8')
  except FileNotFoundError:
  ...
  sw_file.close()
  if not os.path.exists(self.this_path + '\\msyh.ttc'):
 ...
 def do(self, arg):
  ...
  while 1 and not force_exit:
   ...
   try:
    ...
    time.sleep(0.5)
   except (se.common.exceptions.WebDriverException, se.common.exceptions.InvalidSessionIdException, urllib3.exceptions.MaxRetryError):   #出现异常说明浏览器已退出
    driver.quit()
    driver = None
