...
cpp_kw = ['C++', 'c++']
game_kw = ['端游', '手游', '游戏', 'UE4', 'ue4', 'Ue4', '虚幻', 'cocos', 'Cocos', 'COCOS', 'TA', 'Ta', '技术美术', '技美', 'Laya', 'laya', 'LAYA', 'Egret', 'egret', 'EGRET']
...
web_kw = ['WEB', 'web', 'Web', '网页', 'html', 'HTML', 'Html', 'css', 'CSS', 'Css', 'JavaScript', 'javascript', 'Javascript', '网站', '前端', 'JS', 'js', 'Js', 'Angular', 'angular']
...
absolute_kw = ['flash', 'Flash', 'FLASH', 'PS', 'Ps', 'ps', 'CG', 'cg', 'Cg', 'VR', 'Vr', 'vr', 'AR', 'Ar', 'ar', 'TA', 'Ta', 'UI', 'Ui', 'ui']
...
class Checking():
 ...
 def do_check(self):
  ...
  if tmp > 0:
   ...
   ws_tmp.append(l)
   delete_row_index = 0
   delete_row_len = len(delete_row)
   for k in range(2, max_r + 1):   #拷贝第二行之后的
    if self.stop_check():   #提供停止
     self.finish_check()    #结束查重
     return
    self.start_update_program_bar((max_r+k)/3, max_r*2/3)   #更新进度条，第二部分
    if delete_row_index < delete_row_len and k == delete_row[delete_row_index]:
     delete_row_index += 1
    else:       #将不含delete字样的行移到新表格
     l.clear()
     r = ws[str(k)]
     for z in r:
      l.append(z.value)
...
class Visualization():
 ...
 def gd_Map(self, kw_list, max_num, min_num, html_name):  #广东（各类）职位数量分布
  ...
  html_str = html_str.replace('https://assets.pyecharts.org/assets/maps/guangdong.js', os.getcwd() + '\\guangdong.js')  #地图轮廓文件改到本地获取
  html_f.close()
 ...
 def gz_Map(self, kw_list, max_num, min_num, html_name):   #广州（各类）职位数量分布
  ...
  html_f.seek(0)      #指针指到开头
  html_str = html_f.read()
  html_str = html_str.replace('https://assets.pyecharts.org/assets/maps/guang3_dong1_guang3_zhou1.js', os.getcwd() + '\\guang3_dong1_guang3_zhou1.js')  #地图轮廓文件改到本地获取
  html_f.close()
   ...
   if ws['A'+str(i)].value != None:
    ...
    while j < len(kw_list):
     if kw_list[j] in absolute_kw and kw_list[j] in jieba.lcut(tmp, cut_all = False):  #判断是否含有关键词
      ...
      j += 1
    if j < len(kw_list):  #是否为相应职位
     if ws['L'+str(i)].value != None and ws['L'+str(i)].value.find('少于') != -1:
      size[0] += 1
     elif ws['L'+str(i)].value != None and ws['L'+str(i)].value.find('50-150') != -1:
      size[1] += 1
     elif ws['L'+str(i)].value != None and ws['L'+str(i)].value.find('150-500') != -1:
      size[2] += 1
     elif ws['L'+str(i)].value != None and ws['L'+str(i)].value.find('500-1000') != -1:
      size[3] += 1
 ...
 def eachJob_Req_WordCloud(self, KW_list_list, pic_name_list):
  global ws
  global absolute_kw
...
深厚 深刻 其他 其它 部门 规定 购买 社保 保险 聘 请 急 ü ● ★ ◆ ■ 丰富 富 周末 双休 者 能够 人员 具 分红 发放 广阔 一门 一次 一个 。 ° 一次 年终奖 周一 座 大厦 免费 或"""   #停用词表
  ...
  if self.stop():  #提供停止
   return
  wc = WordCloud(font_path = os.getcwd() + '\\msyh.ttc', max_words = 1600, max_font_size = 120, min_font_size = 8, width = 1280, height = 800,
 ...
 def do(self):
  ...
  if self.ui.gdEachJobvar.get() == 1:                 #广东省数媒相关行业各职位数量分布
   ...
   if self.stop():  #提供停止
    self.finish()
    return
   self.gd_Map(animate_kw, 90, 0, '广东省动画相关职位数量分布')
   if self.stop():  #提供停止
    self.finish()
    return
   self.gd_Map(cs_kw, 80, 0, '广东省C#相关职位数量分布')
   if self.stop():  #提供停止
    self.finish()
    return
   ...
   if self.stop():  #提供停止
    self.finish()
    return
   self.gd_Map(game_kw, 100, 0, '广东省游戏相关职位数量分布')
   if self.stop():  #提供停止
    self.finish()
    return
   ...
   if self.stop():  #提供停止
    self.finish()
    return
   self.gd_Map(media_kw, 600, 0, '广东省媒体相关职位数量分布')
   if self.stop():  #提供停止
    self.finish()
    return
   ...
   if self.stop():  #提供停止
    self.finish()
    return
   self.gd_Map(technology_kw, 120, 0, '广东省数媒技术相关职位数量分布')
   if self.stop():  #提供停止
    self.finish()
    return
   self.gd_Map(unity_kw, 20, 0, '广东省Unity相关职位数量分布')
   if self.stop():  #提供停止
    self.finish()
    return
   self.gd_Map(virtual_kw, 10, 0, '广东省虚拟现实相关职位数量分布')
   if self.stop():  #提供停止
    self.finish()
    return
...
#-----------------------------------------------------------------------------------------------
  if self.ui.eachJobEduvar.get() == 1:               #数媒行业各职位学历要求分布
   ...
   if self.stop():  #提供停止
    ...
    return
#------------------------------------------------------------------------------------------------
  if self.ui.eachJobCSizevar.get() == 1:               #数媒行业各职位公司大小分布
   if self.stop():  #提供停止
    self.finish()
    return
...
#-----------------------------------------------------------------------------------------------
  if self.ui.eachJobReqvar.get() == 1:               #数媒行业各职位描述和要求词云图
   if self.stop():  #提供停止
    self.finish()
    return
   self.eachJob_Req_WordCloud(tmp_kw_list_list, file_name_list)
   if self.stop():  #提供停止
    self.finish()
    return
#-----------------------------------------------------------------------------------------------
...
class MainUi(UiRoot):
 ...
 def __init__(self):
  ...
  img1 = PhotoImage(file = os.getcwd() + '\\google.gif')
  img2 = PhotoImage(file = os.getcwd() + '\\boss.gif')
  img3 = PhotoImage(file = os.getcwd() + '\\51job.gif')
