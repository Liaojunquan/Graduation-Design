...
Ar 虚拟仿真 Unreal 交互开发 程序开发 计算机图形 Flash flash FLASH An AN MG mg Mg 二维 分镜 AE Ae ae PR pr Pr 达芬奇 多媒体 Davinci 调色 修图 调光 图片 数字媒体 算法 大数据 
数据挖掘 .NET .net .Net C# 游戏 TA ZB"""     #关键词
...
class Crawling():
 ...
 def pause(self):     #暂停函数
  ...
  global stopFlag
  global startFlag
  while startFlag == True and runFlag == False and stopFlag == False:
   #self.ui.log('暂停中')
   time.sleep(1)      #休眠实现暂停
 ...
 def append_list_job(self, ee, ee_l, ee_d, ee_fl, ee_c, ee_link, ee_t_s, ee_b, ee_time, ee_msg):  #职位名称  薪资  地区|经验|学历  福利  公司名称  链接  公司类型|大小  业务方向 发布时间 职位描述和要求
  ...
  sal_avg = 0
  self.pause()         #提供暂停
#  ee_tmp = copy.deepcopy(ee)   #拷贝职位名称字符串
#  if ee_tmp.find('月薪') != -1 and ee_tmp.find('K') != -1:
#   ee_tmp = ee_tmp[:ee_tmp.find('月')] + ee_tmp[ee_tmp.find('K')+1:]    #去掉薪酬字符
#  elif ee_tmp.find('月薪') != -1 and ee_tmp.find('k') != -1:
#   ee_tmp = ee_tmp[:ee_tmp.find('月')] + ee_tmp[ee_tmp.find('k')+1:]
#  elif ee_tmp.find('月薪') != -1 and ee_tmp.find('千') != -1:
#   ee_tmp = ee_tmp[:ee_tmp.find('月')] + ee_tmp[ee_tmp.find('千')+1:]
#  elif ee_tmp.find('月薪') != -1 and ee_tmp.find('W') != -1:
#   ee_tmp = ee_tmp[:ee_tmp.find('月')] + ee_tmp[ee_tmp.find('W')+1:]
#  elif ee_tmp.find('月薪') != -1 and ee_tmp.find('w') != -1:
#   ee_tmp = ee_tmp[:ee_tmp.find('月')] + ee_tmp[ee_tmp.find('w')+1:]
#  elif ee_tmp.find('月薪') != -1 and ee_tmp.find('万') != -1:
#   ee_tmp = ee_tmp[:ee_tmp.find('月')] + ee_tmp[ee_tmp.find('万')+1:]
#  if ee_c.strip().find('深圳市腾讯计算机系统有限公司') != -1:
#   ee_tmp = ee_tmp[ee_tmp.find('-')+1:]
#   ee_tmp = ee_tmp.replace(' ','')
#   if ee_tmp.find('（') != -1:
#    ee_tmp = ee_tmp[:ee_tmp.find('（')+1]
#    ee_tmp = ee_tmp.replace('（','')
#   elif ee_tmp.find('(') != -1:
#    ee_tmp = ee_tmp[:ee_tmp.find('(')+1]
#    ee_tmp = ee_tmp.replace('(','')
#  for index_delWord in self.delWord:
#   ee_tmp = ee_tmp.replace(index_delWord, '')  #去除职位名称中不必要的字符
  ...
  l.append(ee_msg.replace('\n','').replace('【',' ').replace('】',' ').replace('"','').replace('  ','').replace('急聘','').replace('-','').replace('.','').replace('*','').strip())   #职位描述和要求
  self.ui.log('---' + ee + '---')   #打印职位
  return l
...
#---------------------------------------------------------------------以上为爬取Boss直聘------------------------------------------------------------------------------------------------------------
def p_self(ws_, range_min, range_max, queue_, now_time):  #自查重子进程
...
class Checking():
 ...
 def delete_not_gd(self, str_qu):   #检查地区是否为非广东
  global checkGD
  if checkGD == True:   #去除非广东数据
   if str_qu == None:
    return True
   gd_city_name = ['广州', '深圳', '东莞', '佛山', '珠海', '惠州', '中山', '江门', '汕头', '湛江', '肇庆', '揭阳', '清远', '潮州', '梅州', '茂名', '韶关', '阳江', '河源', '汕尾', '云浮', '广东']
   index_tmp_city = 0
   while index_tmp_city < 22:
    if str_qu.find(gd_city_name[index_tmp_city]) == -1:
     index_tmp_city += 1
    else:
     break
 ...
 def do_check(self):
  ...
  if p_n > 1:      #多进程
   ...
   for x in range(2, p_n+1):
    pool.apply_async(p_self, (ws, (x-1)*max_r//p_n+1, x*max_r//p_n+1, queue, now_time))   #多进程自查重
   for i in range(2, max_r//p_n + 1):
    for j in range(i+1, max_r//p_n + 1):
     self.start_update_program_bar(i//2, max_r//p_n)   #更新进度条，第一部分
     if ws['A'+str(i)].value == 'delete':    #遇到delete字符则跳过本趟j从i+1到max_r+1的循环
      break
   ...
   for x in range(1, p_n+1):
    for y in range(x+1, p_n+1):
     if x == 1:
      if y != 2:
       pool.apply_async(p_each, (ws, 2, x*max_r//p_n+1, (y-1)*max_r//p_n+1, y*max_r//p_n+1, queue, now_time))
     else:
      pool.apply_async(p_each, (ws, (x-1)*max_r//p_n+1, x*max_r//p_n+1, (y-1)*max_r//p_n+1, y*max_r//p_n+1, queue, now_time))
   for i in range(2, max_r//p_n+1):
   ...
   pool = Pool(1)
   queue = multiprocessing.Manager().Queue()
   pool.apply_async(p_self, (ws, 2, max_r//2+1, queue, now_time))
   for i in range(max_r//2+1, max_r + 1):
    for j in range(i+1, max_r + 1):
     self.start_update_program_bar((i-max_r//2)//2, max_r//2)   #更新进度条，第一部分
     if ws['A'+str(i)].value == 'delete':    #遇到delete字符则跳过本趟j从i+1到max_r+1的循环
      break
     if self.stop_check():   #提供停止
      self.finish_check()    #结束查重
     ...
     if ws['A'+str(i)].value != None and ws['A'+str(j)].value != None and ws['A'+str(i)].value.replace(' ','') == ws['A'+str(j)].value.replace(' ','') and ws['I'+str(i)].value == ws['I'+str(j)].value and ws['E'+str(i)].value == ws['E'+str(j)].value and ws['F'+str(i)].value == ws['F'+str(j)].value: #职位名称 公司名称 地区 经验要求均相同
      if ws['N'+str(i)].value == ws['N'+str(j)].value:   #发布日期相同，包括今日
       tmp_rate = None
       if ws['O'+str(i)].value != None and ws['O'+str(j)].value != None:
        tmp_rate = self.match_rate(ws['O'+str(i)].value, ws['O'+str(j)].value)  #获取匹配率
       elif ws['O'+str(i)].value == None and ws['O'+str(j)].value == None:
        tmp_rate = self.match_rate("", "")  #获取匹配率
       elif ws['O'+str(i)].value == None and ws['O'+str(j)].value != None:
        tmp_rate = self.match_rate("", ws['O'+str(j)].value)  #获取匹配率
       elif ws['O'+str(i)].value != None and ws['O'+str(j)].value == None:
        tmp_rate = self.match_rate(ws['O'+str(i)].value, "")  #获取匹配率
       if tmp_rate == 1.0:          #匹配率100%相同
        ws['A'+str(i)] = 'delete'    #职位描述及要求相同
        self.ui.log('删除重复行: ' + str(i) + ' 和 ' + str(j) + ' 职位描述和要求相似率 = ' + str(tmp_rate*100) + '%')
        tmp += 1
   ...
   pool.close()
   pool.join()
   for i in range(2, max_r//2+1):
    for j in range(max_r//2+1, max_r + 1):
     self.start_update_program_bar(i//2+max_r//4, max_r//2)   #更新进度条，第一部分
     if ws['A'+str(i)].value == 'delete':    #遇到delete字符则跳过本趟j从i+1到max_r+1的循环
      break
     if self.stop_check():   #提供停止
      self.finish_check()    #结束查重
  ...
  if tmp > 0:
   ...
   for z in r:
    l.append(z.value)
   ws_tmp.append(l)
   for k in range(2, max_r + 1):   #拷贝第二行之后的
    if self.stop_check():   #提供停止
     self.finish_check()    #结束查重
     return
    self.start_update_program_bar((max_r+k)/3, max_r*2/3)   #更新进度条，第二部分
    if ws['A'+str(k)].value != 'delete' and self.delete_not_gd(ws['E'+str(k)].value):   #将不含delete字样的行移到新表格  带或不带广东数据
     l.clear()
     r = ws[str(k)]
     for z in r:
      l.append(z.value)
     ws_tmp.append(l)
   del wb['Sheet']   #删除旧表格
   ws_tmp.title = 'Sheet'  #替换为旧表格的名称
   ws_tmp.freeze_panes = 'A2'  #冻结首行
   ws_n = ws_tmp
   self.ui.log('删除成功!')
  max_r = ws_n.max_row    #获取最大行数
  for i in range(2, max_r + 1):      #查找有没有P标记
   self.start_update_program_bar((2*max_r+i)/3, max_r)   #更新进度条，第三部分
   if ws_n['P'+str(i)].value != None:
    P_index = ws_n['P'+str(i)].value
    for j in range(i+1, max_r + 1):
     if ws_n['P'+str(j)].value == P_index:  #查找到两行都有同一P标记
      self.ui.log('-------------->\n需自行删除的重复行: ' + str(i) + ' 和 ' + str(j) + P_tag[P_index])  #打印出需要自行对比删除行号
      ws_n['P'+str(i)] = None    #清除标记
      ws_n['P'+str(j)] = None
      break
     elif j == max_r and ws_n['P'+str(j)].value != P_index:
      ws_n['P'+str(i)] = None    #清除标记
  end_time = time.time()
...
class Visualization():
 ...
 def gd_Map(self, kw_list, max_num, min_num, html_name):  #广东（各类）职位数量分布
  global ws
  self.ui.log(html_name + '图生成中...')
  gd_path = os.getcwd() + '\\广东'
  if os.path.exists(gd_path) == False:  #判断广东文件夹是否存在
   os.mkdir(gd_path)
 ...
 def gz_Map(self, kw_list, max_num, min_num, html_name):   #广州（各类）职位数量分布
  global ws
  self.ui.log(html_name + '图生成中...')
  gz_path = os.getcwd() + '\\广州'
  if os.path.exists(gz_path) == False:  #判断广州文件夹是否存在
   os.mkdir(gz_path)
  ...
  if self.stop():  #提供停止
   return
  gd_map = Map(init_opts = opts.InitOpts(width = '900px', height = '700px'))
  gd_map.add("", [list(z) for z in zip(area, value)], '广州', zoom = 1.2)
  gd_map.set_global_opts(title_opts = opts.TitleOpts(title = html_name, pos_left = 'center', pos_top = '2'),
                    visualmap_opts = opts.VisualMapOpts(max_= (lambda v: max(value) if v == None else max_num)(max_num), min_= (lambda v: min(value) if v == None else min_num)(min_num), is_piecewise = False),
                    toolbox_opts = opts.ToolboxOpts(is_show = True, feature = {'saveAsImage': {}}))
  gd_map.set_series_opts(label_opts = opts.LabelOpts(is_show = True, formatter = '{b} {c}'))
  gd_map.render(gz_path + '\\' + html_name + '.html')
  time.sleep(2)
  if self.stop():  #提供停止
   return
 ...
 def sz_Pie(self, kw_list, html_name):   #深圳（各类）职位数量分布
  global ws
  self.ui.log(html_name + '图生成中...')
  sz_path = os.getcwd() + '\\深圳'
  if os.path.exists(sz_path) == False:  #判断深圳文件夹是否存在
   os.mkdir(sz_path)
 ...
 def gd_Money_Bar(self, kw_list):
  ...
  line.render('广东城市数媒行业职位薪水对比.html')
 ...
 def eachJob_Money_Bar(self, kw_list_list):
  ...
  for i in range(2, max_r+1):
   ...
   if ws['A'+str(i)].value != None:
    tmp = ws['A'+str(i)].value
    for each_index in range(len(kw_list_list)):  #判断是那个职位
     j = 0
     while j < len(kw_list_list[each_index]):
      if tmp.find(kw_list_list[each_index][j]) != -1:
       break
      else:
       j += 1
     if j < len(kw_list_list[each_index]):  #是数媒行业其中的职位
      if ws['B'+str(i)].value != None and ws['C'+str(i)].value != 'null' and ws['D'+str(i)].value != 'null' and type(ws['B'+str(i)].value) != str:
  ...
  for z in range(16):
   ...
   if count[z] != 0:
    ...
    avg_min.append(0)
  del(sum_avg, sum_max, sum_min, count)
  job = ['Android', '动画', 'C#', 'C++', '游戏', '平面', 'Java', '媒体', '建模', '摄影摄像', '后期', '数据库', '数媒技术', 'Unity', '虚拟现实', 'Web网页前端']
  ...
  job_bar.render('数媒行业各职位薪水对比.html')
 ...
 def eachJob_Exp_Pie(self, kw_list, html_name):
  global ws
  self.ui.log(html_name + '图生成中...')
  jy_path = os.getcwd() + '\\经验要求'
  if os.path.exists(jy_path) == False:  #判断经验要求文件夹是否存在
   os.mkdir(jy_path)
  ...
  for i in range(2, max_r+1):
   ...
   if ws['A'+str(i)].value != None:
    ...
    j = 0
    while j < len(kw_list):
     if tmp.find(kw_list[j]) != -1:  #判断是否为相应职位
      break
     else:
      j += 1
    if j < len(kw_list):  #是否为相应职位
     if ws['F'+str(i)].value != None and ws['F'+str(i)].value.find('生') != -1:
      exp[0] += 1
     elif ws['F'+str(i)].value != None and ws['F'+str(i)].value.find('无需') != -1:
      exp[1] += 1
     elif ws['F'+str(i)].value != None and ws['F'+str(i)].value.find('1年') != -1:
      exp[2] += 1
     elif ws['F'+str(i)].value != None and ws['F'+str(i)].value.find('2年') != -1:
      exp[3] += 1
     elif ws['F'+str(i)].value != None and ws['F'+str(i)].value.find('3-4年') != -1:
      exp[4] += 1
  ...
  exp_list = ['在校生/应届生', '无需经验', '1年经验', '2年经验', '3-4年经验', '5-7年经验', '8-9年经验', '10年以上经验']
  if self.stop():  #提供停止
   return
 ...
 def eachJob_Edu_Pie(self, kw_list, html_name):
  global ws
  self.ui.log(html_name + '图生成中...')
  xl_path = os.getcwd() + '\\学历要求'
  if self.stop():  #提供停止
   return
  if os.path.exists(xl_path) == False:  #判断学历要求文件夹是否存在
   os.mkdir(xl_path)
   ...
   if ws['A'+str(i)].value != None:
    ...
    if j < len(kw_list):  #是否为相应职位
     if ws['G'+str(i)].value != None and ws['G'+str(i)].value.find('初中') != -1:
      ...
      edu[7] += 1
     elif ws['G'+str(i)].value == None:
      edu[8] += 1
  edu_list = ['初中及以下', '高中', '中专', '中技', '大专', '本科', '硕士', '博士', '无学历要求']
  if self.stop():  #提供停止
   return
 ...
 def p2(ws_, index, q, s_w, kw_list):  #2进程获取分词数据
  text_p2 = ''
  max_r = ws_.max_row
  range_min = 0
  range_max = 0
  ...
  for i in range(range_min, range_max):
   if ws_['A'+str(i)].value != None:
    ...
    j = 0
    while j < len(kw_list):
     if tmp_A.find(kw_list[j]) != -1:  #判断是否为相应职位
      break
     else:
      j += 1
    if j < len(kw_list) and ws_['O'+str(i)].value != None:  #是否为相应职位  以及职位描述和要求不为空
     tmp_O = ws_['O'+str(i)].value
     for z in s_w:
      tmp_O = tmp_O.replace(z, '')
     text_p2 += ' '.join(jieba.cut(tmp_O, cut_all = False))
  q.put(text_p2)

 def eachJob_Req_WordCloud(self, KW_list, pic_name):
  global ws
  self.ui.log(pic_name+'图生成中...')
  self.ui.log('获取职位描述和要求分词中...')
  cy_path = os.getcwd() + '\\职位描述和要求'
  if os.path.exists(cy_path) == False:  #判断职位描述和要求文件夹是否存在
   os.mkdir(cy_path)
  wordCloudStopWord = """职能 类别 良好 的 年假 以上 者 优先 工作 经验 任职 要求 熟练 使用 相关 专业 有 较 及 以上 等 关键字 一定 负责 公司 优先 考虑 独立 任职 资格 具有 具备 具 
职能 好 要求 五险 一金 类别 带薪 年度 年 法定 节假日 善于 毕业 过程 中 常用 强烈 自己 至少 根据 员工 解决 福利 办公 环境 不 限于 限 于 交办 常用 描述 全日制 地点 婚假 
产假 承受 与 对 旅游 并 进行 晋升 遇到 加入 我们 你 我 入职 优先 住房 公积金 假期 薪酬 深厚 深刻 其他 其它 部门 规定 购买 社保 保险 聘 请 急 ü ● ★ ◆ ■ 丰富 富 制 周末 双休"""
  wordCloud_delete_char = [' ','、','“','”','（','）','《','》','【','】','：','！','￥','……','…','？',
            '——','—','·','(',')','{','}','|','[',']',':',';','"','\'','\\','<','>','?',',','.','/',
            '!','`','~','\n','*','+','$','&','%','#','@','^','-','_','=']  #要去除的字符列表
  if self.stop():  #提供停止
   return
  wc_sw = wordCloudStopWord.replace('\n', '').split(' ')
  wc_sw = wc_sw + wordCloud_delete_char
  ...
  for j in range(2):
   if self.stop():  #提供停止
    return
   pool.apply_async(p2, (ws, j, queue, wc_sw, KW_list))
  pool.close()
  pool.join()
  fc_text = ''   #分词字符串
  if self.stop():  #提供停止
   return
  while queue.qsize() != 0:
   fc_text = fc_text + ' ' + queue.get()
  del(manager, queue, pool)
  try:
   sw_file = open('my_stopwords.txt','r', encoding = 'utf-8')
  except FileNotFoundError:
   self.ui.log('当前文件目录'+os.getcwd()+'下找不到停用词文件my_stopwords.txt')
   self.ui.log(pic_name +  '生成失败!')
   return
  if self.stop():  #提供停止
   return
  cn_stopwords = set()
  ...
  if self.stop():  #提供停止
   return
  wc = WordCloud(font_path = '.\\msyh.ttc', max_words = 2000, max_font_size = 70, min_font_size = 1, width = 1000, height = 800,
                prefer_horizontal = 1, random_state = 50, margin = 5, stopwords = cn_stopwords, background_color='white')
  wc.generate_from_text(t)
  if self.stop():  #提供停止
   return
  wc.to_file(cy_path + '\\' + pic_name+'.png')
  del(wc, cn_stopwords, content, sw_file)
  self.ui.log(pic_name + '图生成成功!!!')

 def do(self):
  if self.stop():  #提供停止
   self.finish()
   return
  smkeyWord = """引擎 客户端 端游 手游 游戏 Unity U3D u3d U3d UE4 ue4 Ue4 虚幻 cocos Cocos COCOS 2d 平面设计 三维 建模 渲染 视觉 MAYA Maya maya C4D c4d C4d 3DMAX 3dmax ZBrush 飞手 小编 
2D 3d 3D 美工 美术 UI ui Ui 特效 动画 动作 动效 Lua lua LUA Shader shader SHADER 前端 WEB web Web 贴图 材质 影视 视频 拍摄 摄影 摄像 导演 编导 前期 后期 剪辑 分镜 记者 字幕 
小程序 数据库 DBA sql SQL Sql JAVA Java java C++ c++ Android android 安卓 音频 视频 VR Vr 虚拟现实 AR Ar 网页 网站 unity 算法 数字信号 渲染 绑骨 绑定 合成 CG cg Cg 微电影 地编 
Ar 虚拟仿真 Unreal Flash flash FLASH An AN MG mg Mg 二维 分镜 影视 特效 达芬奇 媒体 Davinci 调色 修图 调光 图片 修图 修片 编辑 技术美术 技美 ta Ta 平面设计 犀牛 Blender blender 3ds 3Ds 
C# TA ZB Zbrush 3Dmax 3DMax 编码 解码 编解码 图像 图形 游戏测试 短视频 文案 公众号 html HTML Html css CSS Css JavaScript javascript Javascript PS ps Ps photoshop Photoshop 漫画 动漫 
广告 小编 小红书 微博 抖音 B站 b站 编辑 Laya laya Egret egret"""
  smkw = smkeyWord.replace('\n', '').split(' ')
  android_keyWord = "Android android 安卓 ANDROID"
  animate_keyWord = "动画 动效 动作 flash Flash FLASH AN An an MG Mg mg 动漫"
  cs_keyWord = "C# c#"
  cpp_keyWord = "C++ c++"
  game_keyWord = "端游 手游 游戏 UE4 ue4 Ue4 虚幻 cocos Cocos COCOS TA Ta ta 技术美术 技美 Laya laya Egret egret"
  graphic_keyWord = "UI ui Ui 界面 平面设计 美工 插画 图案 图标 修图 修片 抠图 PS ps Ps photoshop Photoshop 图片 漫画 Graphic graphic 排版 板式 封面"  #平面
  java_keyWord = "Java java JAVA"
  media_keyWord = "音频 视频 导演 编导 分镜 记者 微电影 媒体 文案 公众号 新闻 广播 编辑 广告 小编 小红书 微博 抖音 B站 b站" #媒体
  model_keyWord = "建模 模型 maya Maya MAYA 3dmax 3DMAX 3DMax 3Dmax 3dMax 3dMAX ZB Zb zb ZBrush zbrush Zbrush ZBRUSH 犀牛 Blender blender 3ds 3Ds 贴图 材质 绑骨 绑定 " #建模
  photography_keyWord = "摄影 摄像 拍摄 录像 photograph 航拍 飞手" #摄影摄像
  post_production_keyWord = "渲染 C4D c4d C4d 特效 影视 后期 剪辑 合成 CG cg Cg 达芬奇 Davinci 调色 调光 字幕 包装 地编 Pipeline pipeline" #影视后期
  sql_keyWord = "sql Sql SQL 数据库 DB DataBase Database database" #数据库
  technology_keyWord = "小程序 算法 数字信号 编码 解码 图像 图形 数据结构"  #数媒技术
  unity_keyWord = "Unity U3D U3d u3d u3D unity UNITY Shader SHADER shader"  #Unity3D
  virtual_keyWord = "AR VR Vr 虚拟现实 Ar 虚拟仿真"  #虚拟现实
  web_keyWord = "WEB web Web 网页 html HTML Html css CSS Css JavaScript javascript Javascript 网站 前端"  #Web网页前端
  if self.stop():  #提供停止
   self.finish()
   return
  android_kw = android_keyWord.replace('\n', '').split(' ')
  animate_kw = animate_keyWord.replace('\n', '').split(' ')
  cs_kw = cs_keyWord.replace('\n', '').split(' ')
  cpp_kw = cpp_keyWord.replace('\n', '').split(' ')
  game_kw = game_keyWord.replace('\n', '').split(' ')
  graphic_kw = graphic_keyWord.replace('\n', '').split(' ')
  java_kw = java_keyWord.replace('\n', '').split(' ')
  media_kw = media_keyWord.replace('\n', '').split(' ')
  model_kw = model_keyWord.replace('\n', '').split(' ')
  photography_kw = photography_keyWord.replace('\n', '').split(' ')
  post_production_kw = post_production_keyWord.replace('\n', '').split(' ')
  sql_kw = sql_keyWord.replace('\n', '').split(' ')
  technology_kw = technology_keyWord.replace('\n', '').split(' ')
  unity_kw = unity_keyWord.replace('\n', '').split(' ')
  virtual_kw = virtual_keyWord.replace('\n', '').split(' ')
  web_kw = web_keyWord.replace('\n', '').split(' ')
  if self.stop():  #提供停止
   self.finish()
   return
  
  if self.ui.gdAllvar.get() == 1:                           #广东省数媒相关行业职位数量分布
   if self.stop():  #提供停止
    self.finish()
    return
   self.gd_Map(smkw, 9999, None, '广东省数媒相关行业职位数量分布')
#-----------------------------------------------------------------------------------------------
  if self.ui.gdEachJobvar.get() == 1:                    #广东省数媒相关行业各职位数量分布
   if self.stop():  #提供停止
    self.finish()
    return
   self.gd_Map(android_kw, 1000, 0, '广东省Android安卓相关职位数量分布')
   if self.stop():  #提供停止
    self.finish()
    return
   self.gd_Map(animate_kw, 250, 0, '广东省动画相关职位数量分布')
#-----------------------------------------------------------------------------------------------
  if self.ui.gzEachJobvar.get() == 1:                    #广州市数媒相关行业各职位数量分布
   if self.stop():  #提供停止
    self.finish()
    return
   self.gz_Map(smkw, 9999, None, '广州市数媒相关行业职位数量分布')
   if self.stop():  #提供停止
    self.finish()
    return
   self.gz_Map(android_kw, 1000, 0, '广州市Android安卓相关职位数量分布')
#-----------------------------------------------------------------------------------------------
  if self.ui.szEachJobvar.get() == 1:                    #深圳市数媒相关行业各职位数量分布
   if self.stop():  #提供停止
    self.finish()
    return
   ...
   if self.stop():  #提供停止
    self.finish()
    return
   self.sz_Pie(media_kw, '深圳市媒体相关职位数量分布')
#-----------------------------------------------------------------------------------------------
  if self.ui.eachCityMoneyvar.get() == 1:               #广东城市数媒行业职位薪水对比
   if self.stop():  #提供停止
    self.finish()
    return
   self.gd_Money_Bar(smkw)
#-----------------------------------------------------------------------------------------------
  if self.ui.eachJobMoneyvar.get() == 1:                #数媒行业各职位薪水对比
   if self.stop():  #提供停止
    self.finish()
    return
   tmp_kw_list_list = []
   ...
   del(tmp_kw_list_list)
#-----------------------------------------------------------------------------------------------
  if self.ui.eachJobExpvar.get() == 1:               #数媒行业各职位经验要求分布
   if self.stop():  #提供停止
    self.finish()
    return
   ...
   if self.stop():  #提供停止
    self.finish()
    return
   self.eachJob_Exp_Pie(android_kw, 'Android安卓相关职位经验要求分布')
#-----------------------------------------------------------------------------------------------
  if self.ui.eachJobEduvar.get() == 1:               #数媒行业各职位学历要求分布
   if self.stop():  #提供停止
    self.finish()
    return
   ...
   if self.stop():  #提供停止
    self.finish()
    return
   self.eachJob_Edu_Pie(android_kw, 'Android安卓相关职位学历要求分布')
#-----------------------------------------------------------------------------------------------
  if self.ui.eachJobReqvar.get() == 1:               #数媒行业各职位描述和要求词云图
   if self.stop():  #提供停止
    self.finish()
    return
   self.eachJob_Req_WordCloud(smkw, '数媒行业职位描述和要求词云')
   if self.stop():  #提供停止
    self.finish()
    return
   self.eachJob_Req_WordCloud(android_kw, 'Android安卓相关职位描述和要求词云')
 ...
 def stop(self):
  ...
  if force_exit == True:
   return True
  if isStopVisualization == True and isVisualization == True and tk.messagebox.askyesno(title = '提示', message = '是否要停止可视化？'):  #可视化运行中并且确定要停止
   return True
  else:
   return False
 ...
 def finish(self):
  ...
  if force_exit == False:
   ...
   self.ui.gdAllBtn.config(state=NORMAL)
...
class VisualizationButton(BackgroundTask):     #可视化开始事件
 ...
 def do(self, arg):
  self.ui.log(str(self.ui.gdAllvar.get()))
  self.ui.log(str(self.ui.gdEachJobvar.get()))
  self.ui.log(str(self.ui.gzEachJobvar.get()))
  self.ui.log(str(self.ui.szEachJobvar.get()))
  self.ui.log(str(self.ui.eachCityMoneyvar.get()))
  self.ui.log(str(self.ui.eachJobMoneyvar.get()))
  self.ui.log(str(self.ui.eachJobExpvar.get()))
  self.ui.log(str(self.ui.eachJobEduvar.get()))
  self.ui.log(str(self.ui.eachJobReqvar.get()))
  global isVisualization
  global isStopVisualization
  if isVisualization == False and isStopVisualization == False:
   self.ui.log("初始化中...")
   self.ui.startStopVirtualBtn.config(text = '停止')
   self.ui.gdAllBtn.config(state=DISABLED)
...
class MainUi(UiRoot):
 ...
 def __init__(self):
  ...
  self.gdAllBtn = Checkbutton(visualizationFrame, text = '广东数媒行业职位数量分布', font = '6', command = None, variable = self.gdAllvar)   #1Map
  self.gdAllBtn.place(x = 6, y = 5)
  self.gdEachJobBtn = Checkbutton(visualizationFrame, text = '广东数媒行业各职位数量分布', font = '6', command = None, variable = self.gdEachJobvar)  #16Map
  self.gdEachJobBtn.place(x = 6, y = 29)
  self.gzEachJobBtn = Checkbutton(visualizationFrame, text = '广州数媒行业各职位数量分布', font = '6', command = None, variable = self.gzEachJobvar)  #16Map
  self.gzEachJobBtn.place(x = 6, y = 53)
  self.szEachJobBtn = Checkbutton(visualizationFrame, text = '深圳数媒行业各职位数量分布', font = '6', command = None, variable = self.szEachJobvar)  #16Pie
  self.szEachJobBtn.place(x = 6, y = 77)
  ...
  self.eachCityMoneyBtn.place(x = 6, y = 101)
  ...
  self.eachJobMoneyBtn.place(x = 6, y = 125)
  self.eachJobExpBtn = Checkbutton(visualizationFrame, text = '数媒行业各职位经验要求对比', font = '6', command = None, variable = self.eachJobExpvar)  #16Pie
  self.eachJobExpBtn.place(x = 6, y = 149)
  self.eachJobEduBtn = Checkbutton(visualizationFrame, text = '数媒行业各职位学历要求对比', font = '6', command = None, variable = self.eachJobEduvar)  #16Pie
  self.eachJobEduBtn.place(x = 6, y = 173)
  self.eachJobReqBtn = Checkbutton(visualizationFrame, text = '数媒行业各职位描述要求词云', font = '6', command = None, variable = self.eachJobReqvar)  #16WordCloud
  self.eachJobReqBtn.place(x = 6, y = 197)
  self.startStopVirtualBtn = Button(visualizationFrame, text = '开始', width = 6, height = 3,   font = '10', command = VisualizationButton(self).run)  #按钮
  self.startStopVirtualBtn.place(x = 310, y = 70)
