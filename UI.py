...
网络管理 导师 施工 亚马逊 广告优化 电梯 PE BD 客户代表 结构 审计 商业化 商业 硬件 客户 媒介 葡萄牙 法语 俄语 商业 意大利 西班牙 德语 韩语 用户研究 信息流 Amazon 产品开发 专柜 导购 绩效 调酒 
...
动力 安防 BI 弱电 资金 理赔 QT 消防 产品专员 5G 网路 生信 信息安全 渗透 OTA 竞价 Facebook IC前端 外卖 速卖通 船务 带团 游戏平台 生物"""  #停用词
...
class Crawling():
 ...
 def append_list_job(self, ee, ee_l, ee_d, ee_fl, ee_c, ee_link, ee_t_s, ee_b, ee_time, ee_msg):  #职位名称  薪资  地区|经验|学历  福利  公司名称  链接  公司类型|大小  业务方向 发布时间 职位描述和要求
  ...
  l.append(ee_msg.replace('\n','').replace('【',' ').replace('】',' ').replace('"','').replace('  ','').replace('急聘','').strip())   #职位描述和要求
  self.ui.log('---' + ee + '---')   #打印职位
  return l
     ...
     if ws['N'+str(i)].value == ws['N'+str(j)].value:   #发布日期相同，包括今日
      ...
      if tmp_rate == 1.0:          #匹配率100%相同
       ...
       if (ws['H'+str(i)].value == None and ws['H'+str(j)].value == None) or (ws['H'+str(i)].value != None and ws['H'+str(j)].value != None):
        ...
        self.ui.log('重复行: ' + str(i) + ' 和 ' + str(j) + ' 职位描述和要求相似率 = ' + str(tmp_rate*100) + '%')
        tmp += 1
      elif tmp_rate >= 0.50:   #匹配率小于70%大于50%，需自行比对
...
#  self.finish_check()
  ...
  if tmp > 0:
   ...
   for k in range(1, max_r + 1):
    ...
    if ws['A'+str(k)].value != 'delete':   #将不含delete字样的行移到新表格
     l.clear()
     r = ws[str(k)]
     for z in r:
      l.append(z.value)
  ...
  self.ui.log('查重完成! 用时' + str(int(time.time()-start_time)) + 's')
  wb.save(file_name)
  self.finish_check()  #结束查重
  wb.close()
